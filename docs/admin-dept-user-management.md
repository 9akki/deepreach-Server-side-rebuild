...
本文档记录了近期针对部门与用户创建流程的约束更新，便于后台管理及接口联调。
...
| `deptType` | string | 是 | 部门类型：`1` 系统、`2` 代理、`3` 买家总账号、`4` 买家子账号。 |
...
| `level` | int | 代理部门建议显式传递 | `deptType=2` 时支持显式指定代理层级（1~3）；若缺省则按照父部门自动推导。其余类型忽略该值。 |
